...
def Add_Sine():
    ...
    plt.xlabel('Time (s)')
    st.write("the max frequency", st.session_state.max_freq)
    return fig
...
def sampling_sine(plot_freq, plot_amplitude, sampling_freq, max_freq):
    ...
    st.write("the sampling frequency", (sampling_freq * max_freq))
    st.plotly_chart(fig)
    return ts, my_data
...
def Upload_Plot(my_data, noise):
    ...
    freq = st.sidebar.slider("Specify frequency", min_value=0, max_value= 8,value=2, step=1)
...
def Add_Samples(freq):
    ...
    fig = plt.figure(figsize=(10, 5))
    #plt.plot(t, a, color='g', linewidth=2)
    ...
    plt.plot(t_rec, a_rec, "x", color='b')
    # plt.plot(t_rec, a_rec,"x", 'b')
...
def home_page():
    home_subhead = ['Sampling:', 'Nequist Sampling Law:']
    home_text = ['Hello and Welcome to this sampling studio',
                 'As a start let us introduce some important Definations'
                 'It is the reduction of a continuous-time signal to a discrete-time signal.',
                 'It sampling technique establishes a sufficient condition for a sample rate that permits a discrete sequence of samples to capture all the information from a continuous-time signal of finite bandwidth.',
                 ]
    st.write(home_text[0])
    st.write(home_text[1])
    st.subheader(home_subhead[0])
    st.write(home_text[2])
    image1 = Image.open('download1.png')
    st.image(image1, caption='sampling technique')
    st.subheader(home_subhead[1])
    st.write(list[3])
    image2 = Image.open('download2.png')
    st.image(image2, caption='Nequist Sampling technique')
...
with st.sidebar:
    selected1 = option_menu(
        ...
        options=['Home Page', 'Sine Signal', 'Upload Signal', 'Contact Us'],
        icons=['house-door-fill', 'reception-4',
               'arrow-bar-up', 'person-lines-fill'],
        # arrows-fullscreen
        menu_icon='activity',
        styles={
...
if selected1 == 'Contact Us':
    ...
    st.image(image3, caption='Nequist Sampling technique')
